...
def resnet3d_retinanet(num_classes, backbone='resnet3d50', inputs=None, modifier=None, **kwargs):
    if inputs is None:
        if keras.backend.image_data_format() == 'channels_first':
            inputs = keras.layers.Input(shape=(3, None, None, None))
        else:
            inputs = keras.layers.Input(shape=(None, None, None, 3))

        # create the resnet backbone
    if backbone == 'resnet3d50':
        resnet = keras_resnet.models.ResNet3D50(inputs, include_top=False, freeze_bn=True)
    elif backbone == 'resnet3d101':
        resnet = keras_resnet.models.ResNet3D101(inputs, include_top=False, freeze_bn=True)
    elif backbone == 'resnet3d152':
        resnet = keras_resnet.models.ResNet3D152(inputs, include_top=False, freeze_bn=True)
    else:
        raise ValueError('Backbone (\'{}\') is invalid.'.format(backbone))
